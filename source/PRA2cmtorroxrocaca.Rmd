...
# Obtenim valors de NAs per columnes:
dfnas <- df3 %>%
  select(everything()) %>%
  summarise_all(funs(sum(is.na(.))))

# Ens quedem amb les columnes que tenen més d'un NA:
dfnas <- dfnas[sapply(dfnas, function(x) any(x > 0))]
kable(dfnas, booktabs = TRUE, 
      caption = "Columnes amb valors NAs filtrant a partir de la temporada 1973-74") %>% 
  kable_styling(latex_options = c("striped", "hold_position"))
```
...
# Establim que si la probabilitat per l'equip és superior a 0.5, ens predigui Playoffs:
prob80s <- ifelse(d80s.prob > 0.5, "Yes", "No")

# Creem la matriu de confusió:
mat80s <- table(prob80s, test80s$Playoffs.f)

# Fem el mateix amb les altres dècades. Comencem pels 90:
d90s.prob = predict(d90s.model, test90s, type="response")
prob90s <- ifelse(d90s.prob > 0.5, "Yes", "No")
mat90s <- table(prob90s, test90s$Playoffs.f)

# Anys 2000:
d00s.prob = predict(d00s.model, test00s, type="response")
prob00s <- ifelse(d00s.prob > 0.5, "Yes", "No")
mat00s <- table(prob00s, test00s$Playoffs.f)

# Anys 2010 fins l'actualitat:
d10s.prob = predict(d10s.model, test10s, type="response")
prob10s <- ifelse(d10s.prob > 0.5, "Yes", "No")
mat10s <- table(prob10s, test10s$Playoffs.f)
```
...
# Canviem els valors de "Sí" i "No" per numèrics:
prob80s <- ifelse(d80s.prob > 0.5, 1, 0)

# Fem la corba:
roc80s <- roc(test80s$Playoffs.f, prob80s)
plot.roc(roc80s, legacy.axes = TRUE, percent = TRUE, xlab = "Taxa de falsos positius (%)", ylab = "Taxa de vertaders positius (%)", col = "#fcba03", print.auc = TRUE)
...
# Corba ROC:
prob90s <- ifelse(d90s.prob > 0.5, 1, 0)
roc90s <- roc(test90s$Playoffs.f, prob90s)
plot.roc(roc90s, legacy.axes = TRUE, percent = TRUE, xlab = "Taxa de falsos positius (%)", ylab = "Taxa de vertaders positius (%)", col = "#fcba03", print.auc = TRUE)
...
# Corba ROC:
prob00s <- ifelse(d00s.prob > 0.5, 1, 0)
roc00s <- roc(test00s$Playoffs.f, prob00s)
plot.roc(roc00s, legacy.axes = TRUE, percent = TRUE, xlab = "Taxa de falsos positius (%)", ylab = "Taxa de vertaders positius (%)", col = "#fcba03", print.auc = TRUE)
```
...
# Corba ROC:
prob10s <- ifelse(d10s.prob > 0.5, 1, 0)
roc10s <- roc(test10s$Playoffs.f, prob10s)
plot.roc(roc10s, legacy.axes = TRUE, percent = TRUE, xlab = "Taxa de falsos positius (%)", ylab = "Taxa de vertaders positius (%)", col = "#fcba03", print.auc = TRUE)
```
...
# Creem el dataframe:
allDF <- df %>% filter(decade != "Pre-three era")
allDF$Playoffs.f <- as.factor(allDF$Playoffs)
allDF <- allDF %>% select(Playoffs.f, vars)

# Fem partició entre entrenament i test:
hAll <- holdout(y=allDF$Playoffs.f, ratio=2/3, mode="stratified")
trainAll <- allDF[hAll$tr,]
testAll <- allDF[hAll$ts,]

# Creem el model i la matriu de confusió:
dAll.model = glm(Playoffs.f ~., data = trainAll, family = binomial)

# Creem la predicció i la matriu de confusió sobre els resultats
dAll.prob = predict(dAll.model, testAll, type="response")

# Establim que si la probabilitat per l'equip és superior a 0.5, ens predigui Playoffs:
probAll <- ifelse(dAll.prob > 0.5, "Yes", "No")

# Creem la matriu de confusió:
matAll <- table(probAll, testAll$Playoffs.f)

# Analitzem el model:
kable(matAll, booktabs = TRUE, 
      caption = "Matriu de confusió amb el model per totes les èpoques (post creació tir de 3)", 
      col.names = c("No", "Yes")) %>% 
  kable_styling(latex_options = c("striped", "hold_position"))
sens <- round(sensitivity(matAll), 3) * 100
spec <- round(specificity(matAll), 3) * 100
prec <- round(precision(matAll), 3) * 100
values <- c(sens, spec, prec)
modSum <- data.frame(pars = params, description = desc, val = values)

# Taula resum:
kable(modSum, booktabs = TRUE, 
      caption = "Mesures de la qualitat del model per totes les èpoques (post creació tir de 3)", 
      col.names = c("Mesura", "Descripció", "Valor al model (%)")) %>% 
  kable_styling(latex_options = c("striped", "hold_position"))

# Corba ROC:
probAll <- ifelse(dAll.prob > 0.5, 1, 0)

# Fem la corba:
rocAll <- roc(testAll$Playoffs.f, probAll)
plot.roc(rocAll, legacy.axes = TRUE, percent = TRUE, xlab = "Taxa de falsos positius (%)", ylab = "Taxa de vertaders positius (%)", col = "#fcba03", print.auc = TRUE)
...
```{r}
# Guardem les dades en un dataframe:
sumAll <- tidy(dAll.model)

# Marquem allà on el p-valor sigui inferior a 0.05:
#sumAll$p.value = cell_spec(sumAll$p.value, color = ifelse(sumAll$p.value > 20, "red", "black"))

# Fem la taula:
kable(sumAll, booktabs = TRUE, caption = "Resum del model per a totes les èpoques", 
      col.names = c("Paràmetre", "Estimate", "Error estàndard", "Estadístic", "p-valor")) %>% 
  column_spec(5, color = ifelse(sumAll$p.value < 0.05, "red", "black")) %>% 
  kable_styling(latex_options = c("striped", "hold_position"))
```
...
* **Edat**: sembla que hi ha una relació entre l'edat mitjana de l'equip i arribar a playoffs. Com té un valor de _estimate_ positiu, això ens indica que els equips més veterans tindrien més possibilitats d'arribar a playoffs que els més joves segons el nostre model. 

* **Rebots ofensius**: com en el cas de l'edat, el valor positiu de _estimate_ indica que els equips amb més rebots ofensius per partit ho tindran millor per arribar a playoffs. 

* **Pèrdues**: perdre molt la pilota disminuirà les possibilitats d'arribar a _playoffs_, tal com indica el valor negatiu de _estimate_. 

* **Rating defensiu**: és el predictor amb el p-valor més baix, el que ens indica la seva forta relació amb arribar a playoffs. Veiem que té un valor _estimate_ negatiu, el que vol dir que els equips amb valors més alts d'aquesta variable tindran menys probabilitats d'arribar a playoffs, ja que un valor més petit d'aquesta variable indica una millor defensa. Notablement, el rating ofensiu sembla no tenir cap relació amb arribar a playoffs. Això ens suggereix que el nostre model premia una gran defensa abans que un gran atac. 

* **Dificultat del calendari**: com era d'esperar, tenir rivals més complicats dificulta l'arribada a playoffs. Recordem que a la NBA els equips juguen més amb els de la seva divisió que amb els d'altres divisions. Si tens a la teva divisió rivals complicats, això complicarà els playoffs. El que ens indica el seu _estimate_ negatiu és que a major dificultat de calendari (és un percentatge calculat en funció de les victòries dels rivals), menys probabilitats d'arribar a playoffs. 

Destaquem també la variable _ft_pct_, és a dir, el percentatge de tirs lliures encertats. Amb un valor de _estimate_ molt positiu, es queda prop de tenir un p-valor significatiu (0.078). Això dóna rellevància a l'encert d'aquest tipus de llançaments, que de vegades semblen valorar-se menys que els llançaments de dos o els triples. Per finalitzar, també parlarem de les mètriques avançades _ts_pct_ (o _True Shooting Percentage_, una mètrica d'eficiència en el tir que té en compte l'encert en tot el tipus de llançaments, amb fórmula _FGA + 0.44 * FTA_) i _efg_pct_ (tirs de camp efectius, on es dóna més valor al llançament de tres i que té com a fórmula (FG + 0.5 * 3P) / FGA). Són dues mètriques amb valors _estimate_ molt positius, el que clarament posa de manifest la seva importància, però amb p-valor molt per sobre de 0.5, el que ens diu que no són bons predictors segons el nostre model. 

# Exercici 5: Representació dels resultats
La representació gràfica dels resultats per a cada anàlisi es pot trobar al llarg de l'exercici 4. 

# Exercici 6: Resolució del problema.

El conjunt de dades que presentem es basa en desenes de mètriques estadístiques sobre les temporades de l'NBA i de la WNBA. Aquestes es van recollir durant la primera pràctica a partir de l'ús de tècniques de _web scrapping_. Aquest conjunt presenta 1900 observacions i 53 variables. En aquest, podem trobar des d'estadístiques bàsiques fins a altres més complexes sobre el joc d'un equip durant la temporada regular.
...
# Exercici 7: Codi.
El codi de la pràctica es troba en l'arxiu R Markdown 'PRA2cmtorroxrocaca.Rmd' pujat a la carpeta `source` del nostre GitHub: https://github.com/ghcmt/PRA1TCVDUOC. No el reproduïm aquí per falta d'espai. Cal tenir en compte, que s'ha realitzat la pràctica sobre el mateix repositori. En conseqüència, trobarem que aquest s'anomena PRA1 però conte tant els fitxers de la primera pràctica en una carpeta per separat i els fitxers de la segona més visibles.

# Exercici 8: Vídeo.
L'enllaç al vídeo es troba aquí: https://drive.google.com/file/d/1nIOK3WWEip6KktFsJo3WuuQ3iUHBh-sv/view?usp=share_link . 


# Generació de CSV amb les dades utilitzades
El fitxer CSV amb les dades utilitzades s'anomena NBA_WNBA_PRA2.csv i es troba a la carpeta _dataset_ del nostre Github. 

```{r}
# Agafem les variables:
dfCSV <- df %>% filter(decade != "Pre-three era")
dfCSV <- dfCSV %>% select(Season, League, Team, Playoffs, win_loss_pct, fg, fg2, fg3, ft, pf, vars)

# Ho guardem en un dataset:
write.csv(dfCSV, "../dataset/NBA_WNBA_PRA2.csv")
```

\newpage

# Taula de contribucions
A continuació afegim la taula de contribucions a la pràctica:

```{r}
contribs <- c("Investigació prèvia", "Redacció de les respostes", "Desenvolupament del codi", "Participació al vídeo")
parts <- rep(c("Carlos Martínez Torró, Xavier Roca Canals"), 4)

contDF <- data.frame(conts = contribs, partic = parts)
