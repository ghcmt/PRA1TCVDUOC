...
Per a seleccionar les variables a utilitzar, mirarem d'afegir el % d'encerts per les estadístiques de tirs de camp. Aquesta ens permet entendre de millor el rendiment de l'equip sobre aquesta estadística. Així i tot, continuarem usant certes variables d'anteriors anàlisis.
...
df80s <- select(df80s, Playoffs.f, age, fg_pct, fga, fg3_pct, fg3a, ft_pct, orb, drb, ast, stl, tov, off_rtg, def_rtg)
df90s <- select(df90s, Playoffs.f, age, fg_pct, fga, fg3_pct, fg3a, ft_pct, orb, drb, ast, stl, tov, off_rtg, def_rtg)
df00s <- select(df00s, Playoffs.f, age, fg_pct, fga, fg3_pct, fg3a, ft_pct, orb, drb, ast, stl, tov, off_rtg, def_rtg)
df10s <- select(df10s, Playoffs.f, age, fg_pct, fga, fg3_pct, fg3a, ft_pct, orb, drb, ast, stl, tov, off_rtg, def_rtg)
...
En aquest cas, podem observar les dimensions dels primers subconjunts que fan referència a l'època dels 80. Trobem per al subconjunt d'entrenament 154 files amb un total de 14 variables. Així doncs, per al subconjunt de prova, trobem 77 files i 14 variables.

### Anàlisi 3.3: Creació del model i avaluació a partir de la utilització de la regressió logística.

El mètode de model supervisat seleccionat es tracta del model de regressió logística. Aquest ens permetrà predir i classificar la variable categòrica _Playoffs_ per tal de determinar si un equip pot entrar a aquests segons les seves estadístiques. Primer de tot, crearem el model a partir del subconjunt d'entrenament i finalment, mirarem de realitzar la predicció amb el subconjunt de prova per tal d'avaluar-lo. Per avaluar, crearem una matriu de confusió per cada una de les èpoques.
...
# Creem la predició i la matriu de confusió sobre els resultats
...
```{r}
mat80s
perc<-100 * sum(diag(mat80s)) / sum(mat80s)
print(sprintf("El %% de registres correctament classificats és: %.4f %%",perc))
```

```{r}
mat90s
perc<-100 * sum(diag(mat90s)) / sum(mat90s)
print(sprintf("El %% de registres correctament classificats és: %.4f %%",perc))
```

```{r}
mat00s
perc<-100 * sum(diag(mat00s)) / sum(mat00s)
print(sprintf("El %% de registres correctament classificats és: %.4f %%",perc))
```

```{r}
mat10s
perc<-100 * sum(diag(mat10s)) / sum(mat10s)
print(sprintf("El %% de registres correctament classificats és: %.4f %%",perc))
```
